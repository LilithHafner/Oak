import random#For MUS finding only
from Turing_machine import Machine#For validation and dead simmulations
from sat_gen import save#To export SAT problem to external sorces
from pycosat import solve#SAT SOLVER
...
examples = {'first':{'memory':5, 'time':7, 'input':[1,1,1], 'output':[0,1,1,1]}
            'second':{'memory':8, 'time':10, 'input':[1,1,1,1,1,1], 'output':[0,1,1,1,1,1,1]}}
...
# but this aint publishable yet so...)
...
def export():
    return {
        'examples':[
                {'name':e,
                    'tape':[[Variable('T',e,t,x) for t in range(examples[e]['time'])] for x in range(examples[e]['memory'])]
                } for e in examples
            ]
    }
